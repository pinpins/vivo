...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total vivo balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized vivo balance of the wallet\n"
...
UniValue debug(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "vivo|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"vivoaddress\"\n"
            "\nReturn information about the given vivo address.\n"
            "\nArguments:\n"
            "1. \"vivoaddress\"     (string, required) The vivo address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"vivoaddress\", (string) The vivo address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Vivo address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are vivo addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) vivo address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"vivoaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"vivoaddress\"  (string, required) The vivo address to use for the signature.\n"
